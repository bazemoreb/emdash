...
- `fixtures` creates site, collection, field, user, content, binary media, and plugin state without firing hooks.
...
- `inspect` reads content, plugin storage, KV, settings, plugin state, scheduled tasks, media metadata and bytes, comments, and captured email.
...
The runtime host exposes shipped operations only. Capability-specific helpers for translations, publication policy, Block Kit interactions, binary HTTP, raw routes, taxonomies, redirects, expanded comment administration, and encrypted settings belong to the releases that add those capabilities.
