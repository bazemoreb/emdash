...
- `inspect` reads content, plugin storage, KV, raw persisted setting envelopes, plugin state, scheduled tasks, media, comments, and captured email.
...
The runtime host exposes shipped operations only. Capability-specific helpers for translations, publication policy, Block Kit interactions, binary HTTP, raw routes, taxonomies, redirects, expanded comment administration, and media bytes belong to the releases that add those capabilities.
