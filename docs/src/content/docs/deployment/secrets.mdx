...
EmDash encrypts new and re-saved values with the first key. It uses the stored `kid` fingerprint to select an older key for reads. Re-save every plugin secret before removing the old key, then verify those integrations from a deployment that contains only the new key.
...
Settings a plugin declares with `type: "secret"` (API keys for email providers, form CAPTCHAs, etc.) are entered in the admin UI and encrypted in the `options` table under `plugin:<id>:settings:<key>`. The admin receives only whether a secret is set. Plugin code reads the plaintext through `ctx.settings` inside its isolated runtime.
