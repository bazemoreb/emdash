...
The plugin bridge reads this Worker secret binding directly. The generated admin settings route reads it through `process.env`, so keep `nodejs_compat_populate_process_env` in the Worker compatibility flags. Existing Cloudflare projects must add that flag before saving encrypted settings.
