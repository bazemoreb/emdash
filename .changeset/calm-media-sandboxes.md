...
Ready-media metadata URLs use an authenticated media ID route. Administrator sessions and API tokens with `media:read` can fetch the asset without receiving its storage key; logged-out requests are rejected before the route queries media.
